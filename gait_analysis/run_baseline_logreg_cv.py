#!/usr/bin/env python3
"""Run a logistic-regression baseline with stratified cross-validation."""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = argparse.ArgumentParser(
        description="Ejecuta Logistic Regression con validación cruzada estratificada."
    )
    p.add_argument(
        "-i",
        "--input",
...
def main() -> None:
    """Read dataset and evaluate a simple baseline with cross-validation."""
    ...
    for metric in ["test_accuracy", "test_f1_walking", "test_recall_walking"]:
        ...
        print("mean:", round(values.mean(), 4))
        print("std:", round(values.std(), 4))
        print()
